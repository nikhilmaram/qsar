...
def change_vega_script(vega_script_path):
    script = None
    with open(vega_script_path,'r') as f:
        script = f.readlines()
        for index in range(len(script)):
            # print("new line ")
            # print("orig",script[index])
            # time.sleep(2)
            script[index] = re.sub(r'<source>.*</source>','<source>'+os.path.abspath(os.path.normpath('./vega_file/source_test.txt'))+'</source>',script[index])            
            # print(script[index])
            # time.sleep(2)            
            script[index] = re.sub(r'<singleTXT>.*</singleTXT>','<singleTXT>'+os.path.abspath(os.path.normpath('./vega_file/result_test.txt'))+'</singleTXT>',script[index])
            # print(script[index])          
            # time.sleep(2)              
            script[index] = re.sub(r'<multipleTXT>.*</multipleTXT>','<multipleTXT>'+os.path.abspath(os.path.normpath('./vega_file/results'))+'</multipleTXT>',script[index])
            # print("new",script[index])          
            # time.sleep(2)       
...
def switch(smile,epi,vega,test,UUID,testopt=1):
    ...
    if vega:
        ...
        java_command = os.path.normpath("{0}/vega_file/VEGA_CMD/VEGA-CLI.jar -script {1}".format(dir_path,vega_script_path))
