...
def read_epi_result_toJson(resultFilePath, jsonOutputPath="default"):
	...
	for i in range(len(indexesOfRecords)-1):
		...
		for line in linesForOneRecord:
			if "SMILES :" in line:
				...
				while "----------------------------"  not in ttline:
					if "Daphnid" in ttline:
						if "LC50" in ttline and "48-hr" in ttline:
							currentChemical["dmLC50_48hr_ecosar  mg/L"]=ttline[66:].strip()
						if "ChV" in ttline and "-hr" not in ttline:
							currentChemical["dmChV_ecosar  mg/L"]=ttline[66:].strip()

					if "Green Algae" in ttline:
						if "EC50" in ttline and "96-hr" in ttline:
							currentChemical["algaeEC50_96hr_ecosar  mg/L"]=ttline[66:].strip()

					if "Fish" in ttline:
						if "(SW)" in ttline:
							if "LC50" in ttline and "96-hr" in ttline:
								currentChemical["fishLC50SW_96hr_ecosar  mg/L"]=ttline[66:].strip()
							if "ChV" in ttline and "-hr" not in ttline:
								currentChemical["fishChVSW_ecosar  mg/L"]=ttline[66:].strip()##############should be??
						else:
							if "ChV" in ttline and "-hr" not in ttline:
								currentChemical["fishChV_ecosar  mg/L"]=ttline[66:].strip()
							if "LC50" in ttline and "96-hr" in ttline:
								currentChemical["fishLC50_96hr_ecosar  mg/L"]=ttline[66:].strip()

					if "Green Algae" in ttline:
						if "ChV" in ttline and "-hr" not in ttline:
							currentChemical["algaeChV_ecosar  mg/L"]=ttline[66:].strip()

					if "Mysid (SW)" in ttline:
						if "ChV" in ttline and "-hr" not in ttline:
							currentChemical["shrimpSWChV_ecosar  mg/L"]=ttline[66:].strip()
					elif "Mysid" in ttline:
						if "LC50" in ttline and "96-hr" in ttline:
							currentChemical["shrimpLC50_96hr_ecosar  mg/L"]=ttline[66:].strip()

					if "Earthworm" in ttline:
						if "LC50" in ttline and "14-day" in ttline:
							currentChemical["earthworm_14day_ecosar  mg/L"]=ttline[66:].strip()
				...
				while "================="  not in thisline:
					...
					if "Kp (particle/gas partition coef. (m3/ug)):" in thisline:
						...
						if "Mackay model" not in thisline:
							if "not" in nextlinee:
						...
						if "Koa" not in thisline:
							if "not" in nextlinee:
								currentChemical["kAerAir  m3/ug  Koa"]="N/A"
							else:
								currentChemical["kAerAir  m3/ug  Koa"]=str(float(nextlinee[(nextlinee.index(":")+1):].strip()))							
					i=i+1
					thisline=linesForOneRecord[i]
			...
			## will have > <
				...
				while "======================================="  not in thisline:
					...
					if "Log BCF (regression-based estimate):" in thisline:
						...
						currentChemical["bio_HL"]= thisline[thisline.rfind("=")+1:thisline.rfind("days")].strip()
			...
			# elif "Aqueous Base/Acid-Catalyzed Hydrolysis (25 deg C)" in line:
			# 	nextlinee=linesForOneRecord[lineCounter+1]
			# 	nnextlinee=linesForOneRecord[lineCounter+2]
			# 	if "NOT" in nextlinee:
			# 		currentChemical["Kb_HL_pH8  days"]="N/A"
			# 		currentChemical["Kb_HL_pH7  years"]="N/A"
			# 	else:
			# 		currentChemical["Kb_HL_pH8  days"]="N/A"#######################
			# 		currentChemical["Kb_HL_pH7  years"]="N/A"######################
